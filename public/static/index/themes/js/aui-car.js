$(function(){
	...
	$(".plus").click(function() {
        ...
        $.ajax({
            url:judgeNumUrl,
            type:"post",
            data:{id:id,'flag':'add','num':parseInt(t.text()) + 1},
            success:function (data) {
                if(data.status!=200){
                    alert('库存数量不足');
                }else{
    ...
    $(".aui-car-box").each(function() { //循环每个店铺
      ...
      $(this).find(".goodsCheck").each(function() { //循环店铺里面的商品
        if ($(this).is(":checked")) { //如果该商品被选中
            var num = parseInt( $(this).siblings(".aui-car-box-list-text").find(".num").text() ); //得到商品的数量
            var price = parseFloat( $(this).siblings(".aui-car-box-list-text").find(".price").text() ); //得到商品的单价
