...
layui.define('layer' , function(exports){
  ...
  Class.prototype.upload = function(files, type){
    ...
    that.fileLength = function(){
     ...
     //start  console.log(that);  modify by jiangyongfei 2018 /7/26
    if($('#imageShow').size()>0){
      var choseFileNum = $('#imageShow').find('.singer-image').size();//已经选择的文件个数
      if(choseFileNum>0){
          if(options.number && (that.fileLength+choseFileNum) > options.number){
              return that.msg('还能上传'+(options.number-choseFileNum)+'个文件');
          }
      }
    }
    //end
